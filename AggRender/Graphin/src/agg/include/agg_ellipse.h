...
namespace agg
{
    ...
    public:
        ellipse() : 
            m_x(0.0f), m_y(0.0f), m_rx(1.0f), m_ry(1.0f), m_scale(1.0f), 
    ...
    {
        real ra = (FABS(m_rx) + FABS(m_ry)) / 2.0f;
        real da = (real)acos(ra / (ra + 0.125f / m_scale)) * 2.0f;
    ...
    inline unsigned ellipse::vertex(real* x, real* y)
    {
        ...
        *x = m_x + (real)cos(angle) * m_rx;
        *y = m_y + (real)sin(angle) * m_ry;
        m_step++;
